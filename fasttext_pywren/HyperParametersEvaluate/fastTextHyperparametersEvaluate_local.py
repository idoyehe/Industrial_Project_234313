...
def fastText_evaluate(queue, train_path, test_path, hyperparameters_set):
    start = time()
    to_valid_model = fstTxt.train_supervised(train_path, **hyperparameters_set)
    result = to_valid_model.test(test_path)
    end = time()
    queue.put({"precision": result[1], "recall": result[2], "cpu_time": end - start})
